...
export async function askStructured(query: string): Promise<AskResult>{
    ...
    // Keep the instructions brif so that schema stays visible to the model 
    

    const system = "You are a consize assistant. Return only the requested JSON.";
    const user = 
    `Summarize for a beginner:\n` + 
    `"${query}"\n` +
    `Return fileds: summery (short paragraph) , confidence(0..1)`;
